...
# Install to Claude Code (default: user scope)
.\install\install-claude.ps1
.\install\install-claude.ps1 -Scope project
.\install\install-claude.ps1 -Scope local

# Dry-run installers
.\install\install-local.ps1 -WhatIf
.\install\install-claude.ps1 -WhatIf

# Claude Code: validate plugin manifest and load from local dir
...
- **`.claude-plugin/plugin.json`** 鈥?Claude Code manifest. Simpler; requires `description`. Installed via `claude plugin marketplace add` + `claude plugin install`.
...
- **`hooks-adapter.md`** 鈥?Claude Code hooks mapped to Codex-equivalent skill behavior
...
|-----------|----------|--------|
| `plugins/jingyuan/skills/*/SKILL.md` | UTF-8 **without** BOM | Claude Code frontmatter must start at byte 0 |
