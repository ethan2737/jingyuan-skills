...
| code-reviewer | 用户或协调者显式要求审查时 | 两阶段审查：Spec Compliance 和 Code Quality | 并入 `review`；不作为 `dev-builder` 的自动门禁 |
...
- 不并行修改同一文件。
- 产生修改的子任务负责最终验证、对抗性审查和分类 commit；主 Agent 只编排和汇总。
...
- `DONE`：记录实现、验证和 commit；只有显式要求时才进入 review。
