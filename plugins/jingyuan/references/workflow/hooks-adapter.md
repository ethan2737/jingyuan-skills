...
|---|---|---|---|
| detect-feedback-signal.sh | UserPromptSubmit | 检测用户修正、质疑、不满、改进建议 | 对话前置检查；检测到信号后，请求处理完毕后自动进入 `jingyuan-feedback-writer`，写入 `docs/feedback/` |
...
| stop-gate.sh | Stop | 有代码变更但未 review 时阻止停止 | 最终回复前检查 `.jingyuan/needs-review`；如为 `needs_review`，不得声明 Phase 完成，必须提示运行 `jingyuan-code-review` |
