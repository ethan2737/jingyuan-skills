...
<!-- FIRST_PRINCIPLES ADVERSARIAL_REVIEW AUTHOR_COMMITS LOGICAL_INTENT_COMMIT -->

## 共享执行契约

所有角色在执行自身领域流程前后遵守以下契约，角色 Skill 不再重复这些通用规则：

1. **第一性原理**：先明确目标、已验证事实、硬约束、关键假设和未知项，再从目标推导最小方案；不得因为历史流程、已有实现或常见做法而跳过推导。
2. **证据边界**：结论必须区分事实、推断和未知。事实附可复现证据；推断写明依赖的假设；无法验证的内容明确标记，不得包装成确定结论。
3. **对抗性审查**：交付前主动寻找最可能推翻结论的反例、相反证据、边界条件、遗漏依赖和更简单方案。发现问题先修正再交付；仍无法排除时说明风险与影响。
4. **精准执行**：只修改当前任务需要的内容，保护用户已有改动，不顺手重构，不引入未要求的灵活性。
5. **验证闭环**：使用能直接证明目标的命令或人工检查，记录命令、exit code、关键输出和未验证项。旧证据在相关输入或 HEAD 变化后失效。
