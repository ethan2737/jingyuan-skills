...
1. 提取目标用户行为、现有能力、明确不做事项、约束、外部依赖和代码现状。
2. 做 scope challenge：删除不能直接支持当前目标的工作，识别必须由用户决定的分歧。
3. 构建真实依赖顺序；只有存在实际依赖时填写 `Blocked by`。
4. 拆成端到端 vertical slices，每个 slice 必须产生可观察行为，不能只有“建表”“写 API”“做 UI”等水平层。
5. 为每个 slice 写明：Intent、Behavior Contract、范围、关键文件或模块、依赖、测试意图、验证命令、风险、AFK/HITL/Spike/QA 属性。
6. 生成或更新 plan/change 文档；已完成且未受影响的项不重写。
