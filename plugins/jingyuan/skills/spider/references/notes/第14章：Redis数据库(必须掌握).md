...
Redis 是一个基于内存的高效的键值型非关系型数据库，存取效率极高，而且支持多种存储数据结构，使用也非常简单。本节中，我们就来介绍一下 Python 的 Redis 操作，主要介绍 redis-py 这个库的用法。Redis 拥有每秒近10万次读写能力，可以作为我们后续程序设计非常重要的一环。

### 14.1 软件安装

直接把压缩包解压，然后配置一下环境变量就可以了。

![image-20210810184227132](../media/image-20210810184227132.png)



![image-20210810184318301](../media/image-20210810184318301.png)

接下来，在环境变量中将该文件夹配置到path中。

![image-20210810184649037](../media/image-20210810184649037.png)

接下来，我们给redis多配置几个东西，修改redis的配置文件：`redis.windows-service.conf`
...
将 redis 怼到 windows 服务，必须进入到 redis 目录后才可以

![image-20210810185306517](../media/image-20210810185306517.png)
...
![image-20210810185605290](../media/image-20210810185605290.png)

附赠 RDM`redis desktop manager`可以帮我们完成redis数据库的可视化操作

![image-20210810185659813](../media/image-20210810185659813.png)



### 14.2 redis常见数据类型

整个redis的数据库(0)可以看做一个超大号的大字典，想要区分不同的系统，可以在key上做文章。redis中常见的数据类型有5个。
...
red = Redis(host="127.0.0.1",  # 地址
            port=6379,   # 端口
            db=0,   # 数据库
            password=<REDACTED_CREDENTIAL>,  # 密码
...
Redis 还提供了集合存储，集合中的元素都是不重复的，用法如表
...
有序集合比集合多了一个分数字段，利用它可以对集合中的数据进行排序，其用法总结如表
...
Redis 还提供了散列表的数据结构，我们可以用 name 指定一个散列表的名称，表内存储了各个键值对，用法总结如表
