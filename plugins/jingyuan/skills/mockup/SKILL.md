...
[任务]
    读取 docs/PRD/prd.md 和 docs/design/design.md，通过设计工具 MCP 生成完整的设计交付物。确保 PRD 中每个有 UI 的功能都有对应的设计页面，每个页面覆盖所有关键状态变体。
...
[依赖检测]
    ...
    设计文件保存约定：
    - 用户选择 Pencil → 设计稿本体必须保存为 docs/design/ui-design.pen，并在 docs/design/mockup.md 记录该路径、页面 ID、组件清单和截图验证结果
    - 用户选择 Figma → 不生成 .pen 文件，在 docs/design/mockup.md 记录 Figma 文件 URL / file key / 页面 ID / 节点 ID
    - 用户选择其他或跳过 → 只更新 docs/design/mockup.md 的设计稿说明和待处理项
...
[第一性原则]
    ...
    **文档驱动原则**：一切设计决策来自 docs/PRD/prd.md 和 docs/design/design.md。不凭个人偏好发挥，不跳出文档范围添加文档未描述的功能。
...
[设计交付物]
    ...
    **0. 设计文件本体**
    - Pencil 模式：必须创建或更新 docs/design/ui-design.pen
    - Figma 模式：必须记录 Figma 文件和页面定位信息
    - 任意模式都必须同步更新 docs/design/mockup.md，说明设计文件位置、覆盖范围和未覆盖项

    **1. 设计变量**
    从 docs/design/design.md 提取并设置到设计工具中：
...
[工作流程]
    [启动阶段]
        ...
        第二步：加载文档
            读取 docs/PRD/prd.md → 提取所有页面、功能、UI 布局描述、用户流程
            读取 docs/design/design.md → 提取情绪关键词、色彩方向、信息密度、排版方向、交互风格、核心页面视觉备注、状态设计方向
    ...
    [设计阶段]
        ...
        第二步：创建或打开设计文件
            Pencil 模式：打开或创建 docs/design/ui-design.pen，后续所有页面、组件、变量都写入该文件
            Figma 模式：打开用户指定或新建的 Figma 文件，并记录文件 URL / file key / 页面 ID
    ...
    [校验阶段]
        ...
        第四步：输出报告
            更新 docs/design/mockup.md，至少包含：
            - 设计工具：Pencil / Figma / 其他
            - 设计文件位置：Pencil 为 docs/design/ui-design.pen；Figma 为文件 URL / file key
            - 页面 ID、组件 ID、状态变体清单
            - 截图或导出验证结果
            - 未覆盖内容和原因
