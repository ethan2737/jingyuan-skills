...
<!-- METHOD: DISCRIMINATING_HYPOTHESIS -->
...
2. 按 `diagnostics-loop.md` 完成可信失败信号、最小复现、排序假设、单变量实验和双绿门禁；性能问题先记录 baseline。无法建立反馈循环时停止并说明缺失条件。
3. 收集完整错误、调用链、环境、近期变更和组件边界输入/输出；从失败点沿因果链向上追踪，定位坏值、坏状态、坏配置或错误输入首次进入系统的位置，不能停在最终报错点。
4. 把最小复现转成能捕获同一公开行为的失败回归测试，确认按预期失败。
5. 比较最小代码修复、配置修正、回退变更和恢复数据等候选方案，选择风险与验证成本最低且针对根因的方案；高风险问题还要确认撤销修复后信号会重新失败，回归范围按根因传播路径确定。
6. 做对抗性审查，重写当前修复快照并分类提交；状态启用时提交前运行 `CheckCommit`。
...
- 当前结论是否能区分其他合理解释，还是只证明某个补丁暂时让症状消失？
- 修复是否超出 finding 或 bug scope；是否有更小的改动？

无法证明根因时只能报告缓解措施；无法排除关键反例时不得写“已修复”，应标记 `BLOCKED` 或待验证风险。
