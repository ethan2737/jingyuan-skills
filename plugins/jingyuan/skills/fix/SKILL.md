...
2. 构造快速、确定、可重复的失败信号，并优先缩小失败输入、操作序列、环境差异和影响范围；性能问题先记录 baseline。无法构造时停止并说明缺失条件。
3. 收集完整错误、调用链、环境、近期变更和组件边界输入/输出。
4. 从失败点沿因果链向上追踪，定位坏值、坏状态、坏配置或错误输入首次进入系统的位置，不能停在最终报错点。
5. 提出可证伪假设；每个假设写明预测，以及该预测如何区分其他可能原因。每次实验只改变一个变量并记录被证伪原因；连续 3 个假设失败后停止并重新审视架构、环境或需求。
6. 先写能捕获同一公开行为的失败回归测试，确认按预期失败。
7. 比较最小代码修复、配置修正、回退变更和恢复数据等候选方案，选择风险与验证成本最低且针对根因的方案。运行原始失败信号确认转绿，高风险问题还要确认撤销修复后信号会重新失败；回归范围按根因传播路径确定。
