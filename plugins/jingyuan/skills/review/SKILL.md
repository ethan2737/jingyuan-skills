...
1. 固定可复现审查基线：scope、HEAD、目标比较点、merge-base、实际 diff、提交列表、PRD/design/ADR/out-of-scope、验证命令和依赖状态；diff 为空、比较点缺失或 scope 不一致时停止澄清。
...
5. Stage 1 通过后，先读取当前 scope 适用的仓库规范，再执行 Stage 2；区分阻塞问题、应修问题和非阻塞建议，并标明 finding 来自项目硬规则还是工程判断。仓库规则优先，工具已可靠覆盖的问题不重复报告；只有会违反契约、降低代码健康或造成实质风险的问题才能阻塞，个人偏好不得成为 finding。
