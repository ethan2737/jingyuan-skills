...
| 变更类型 | 优先同步对象 | 说明 |
|---|---|---|
| 新增或删除核心功能 | `docs/PRD/PRD.md` 或 `docs/PRD.md`、PRD changelog、`docs/Development-Plan.md` | 先确认这是产品意图，不是临时实现或缺陷。 |
| 修改用户主流程 | PRD、`docs/Design-Document.md`、`docs/Design-Mockup.md`、Development Plan | 主流程变化通常会影响页面、状态、验收和 Phase。 |
| 新增 AI 能力 | PRD 的智能能力需求、PRD changelog、Development Plan、README（如用户需要配置） | 同步触发位置、输入输出、失败状态、成本或权限影响。 |
...
| 文件 | 写什么 | 不写什么 |
|---|---|---|
| `docs/PRD/PRD.md` 或 `docs/PRD.md` | 产品目标、用户流程、功能需求、智能能力、安全和可靠性要求 | 具体数据库字段、接口实现、测试清单、代码结构细节 |
| `docs/PRD/PRD-CHANGELOG.md` 或 `docs/PRD-CHANGELOG.md` | PRD 的新增、修改、删除、重构澄清、关闭待确认 | 没有实际改动的流水账 |
| `docs/Design-Document.md` | 视觉方向、信息密度、交互风格、状态设计方向 | 具体代码实现和 Phase 任务 |
| `docs/Design-Mockup.md` | 设计稿页面清单、组件清单、状态变体、设计文件位置、未覆盖项 | 抽象产品愿景和实现细节 |
...
| 场景 | 处理方式 |
|---|---|
| MCP 可用且差异明确 | 直接更新设计稿，截图或导出验证，再更新 Design-Mockup |
...
```text
同步完成：
- docs/PRD.md：同步了 XX 功能的用户流程和智能能力需求。
