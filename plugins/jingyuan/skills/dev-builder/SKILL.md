...
1. 从 plan/change artifact 定位第一个未完成 checkbox，明确可观察行为、范围、依赖和验收标准。
2. 检查 `git status --short --branch`，识别并保护用户或其他角色已有修改。
3. 记录相关输入和当前 HEAD；确认主语言、运行时、包管理器、测试框架和验证命令。
4. 先建立失败测试或等价可重复信号，再实现满足当前行为的最小改动。
5. 当前 change 内出现失败时，确认是同一目标行为，追到根因后自行最小修复；连续 3 个假设失败则停止并报告架构、环境或需求风险。
6. 运行目标测试、类型检查、构建、lint 及必要的接口或浏览器 smoke，读取完整输出和 exit code。
