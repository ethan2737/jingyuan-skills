...
当项目已有丰富 PRD 但用户未说明是”迭代”还是”重构”时：
1. 先向用户确认意图：是新增/修改/删除需求，还是推翻重写。
2. 如果是推翻重写，检查旧 PRD 中已完成能力和未完成能力，在新 PRD 中保留”现状基线”章节。
3. 明确不做的历史需求写入 `docs/out-of-scope/`。

## 需求发现矛盾

当用户输入的多条需求之间存在逻辑冲突、技术矛盾或资源冲突时：
1. 列出矛盾项：清晰指出冲突的两条或多条需求内容。
2. 分析影响：说明矛盾会导致的范围膨胀、技术不可行或用户体验损伤。
3. 请用户决策：给出 2-3 种解决方向并说明每种的取舍。
4. 在 PRD 中记录决策过程和未采纳的方案，方便后续回溯。

## 没有明确用户/场景

当用户只描述了功能名而没有描述使用者、触发条件和期望结果时：
1. 不自行补全角色和场景。
2. 主动追问：谁会用、在什么场景下触发、期望输出什么。
3. 用已有 PRD 或 `docs/context.md` 中的术语框架引导用户描述。
