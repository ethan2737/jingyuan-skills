...
[开发规则清单]
    ...
    [项目结构规范]
        项目代码放在以项目名命名的子文件夹里，不平铺在根目录。所有规划、产品、设计、反馈和发布说明文档统一收口到 docs/ 目录；JingYuan 工作流状态放在 .jingyuan/。

        ```
        project/
        ├── docs/                  # 产品、设计、计划、反馈等文档统一目录
        │   ├── PRD.md
        │   └── Development-Plan.md
        ...
        │   ├── package.json
        │   └── ...
        └── .jingyuan/              # JingYuan 工作流状态（如 needs-review）
        ```
    ...
    [Git 工作流]
        ...
        Push 策略：
        - 不静默 push。需要 push 时，先展示当前分支、远程地址和将执行的命令，取得用户明确确认
        - push 前确认当前分支正确
        - 如远程仓库未设置 → 提醒用户先配置
    ...
    [进程管理]
        ...
        - 当前终端中的前台服务优先用 `Ctrl+C` 中断
        - 无法交互中断时，使用 PowerShell 停止占用端口的进程，等待 2 秒确保端口释放：
          ```powershell
...
[开发策略]
    ...
    **编码过程中**：
    - 逐个组件对照提取的数值实现
    - 遇到设计稿与 Design Document 冲突时，以设计稿为准
...
[Phase 完成度判断]
    ...
    **四步走**（必须全部通过才能确认 Phase 完成）：
    ...
    - 如无 Playwright → API endpoint 用 `Invoke-WebRequest -UseBasicParsing` 检查返回 200 + 提醒用户在浏览器手动确认 UI 渲染
    - 输出证据：启动日志 + API 响应 + 设计数值对比结果

    **冒烟测试**（四步走之外的额外检查）：
    - 安全扫描：npm audit 无 critical 漏洞
    - 无暴露密钥：PowerShell `Select-String` 检查代码中无硬编码的 API Key、Token
...
[工作流程（初始化模式）]
    ...
    [项目搭建阶段]
        ...
        3. 创建 GitHub **private** 仓库前，展示仓库名、可见性、远程地址、当前分支和计划执行的命令，等待用户明确确认
        4. 首次 commit 后，如需 push，展示 `git push` 命令并等待用户明确确认
